...
外部画像は受信時に20MB、デコード前に一辺8192px・合計1600万pxを上限として検証します。Notion本文の復元も、最大深度20・合計10,000ブロック・300 APIページ・本文1MBを上限とし、上限超過時は安全に中断します。
...
- `GET /wallpaper-analyses/manual-data/{contextHash}`: 手動傾向分析用データJSONをダウンロード
